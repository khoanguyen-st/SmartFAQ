...
_vi_common_words = {
    "la","toi","ban","anh","chi","em","cai","con","mot","hai","ba","muon","khong","co",
    "duoc","nguoi","hoc","truong","day","nay","hoi","cho","lam","ve","ai","may",
    "chao","xin","cam","on","duong","dang","viet","ngon","ngu","nam"
}
...
def _has_diacritics(s: str) -> bool:
    return bool(re.search(r"[àáảãạâầấẩẫậăằắẳẵặèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợ"
                          r"ùúủũụưừứửữựỳýỷỹỵđ]", s, re.IGNORECASE))
...
def detect_language_enhanced(text: str, llm_wrapper=None, async_: bool = False) -> str:
    ...
    if re.search(r"[àáảãạâầấẩẫậăằắẳẵặ"
                 r"èéẻẽẹêềếểễệ"
                 r"ìíỉĩị"
                 r"òóỏõọôồốổỗộơờớởỡợ"
                 r"ùúủũụưừứửữự"
