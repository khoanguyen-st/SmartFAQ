...
const MOCK_DOCUMENTS: DocumentCardData[] = [
...
const sortDocuments = (docs: DocumentCardData[]): DocumentCardData[] => {
...
const UploadedPage = () => {
  const [documents, setDocuments] = useState<DocumentCardData[]>(MOCK_DOCUMENTS)
  const [documentToDelete, setDocumentToDelete] = useState<DocumentCardData | null>(null)
  ...
  const handleOpenDeleteModal = useCallback((doc: DocumentCardData) => {
  ...
  const handleConfirmDelete = useCallback(() => {
    if (documentToDelete) {
      console.warn(`[ACTION] CONFIRMED Deleting document ID: ${documentToDelete.id} - ${documentToDelete.title}`)
  ...
  const handleViewDocument = useCallback((doc: DocumentCardData) => console.warn(`View ${doc.title}`), [])
  const handleReuploadDocument = useCallback((doc: DocumentCardData) => console.warn(`Re-upload ${doc.title}`), [])
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        {placeholders.map(key => (
          <div key={key} className="pointer-events-none opacity-0">
            <DocumentCard
              doc={{ id: key, title: 'Placeholder', date: '', sources: 0 }}
              onDelete={() => {}}
              onView={() => {}}
              onReupload={() => {}}
            />
