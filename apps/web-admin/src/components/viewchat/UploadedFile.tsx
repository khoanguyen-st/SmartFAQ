...
import { IUploadedFile } from '@/lib/knowledge-api.ts'
...
const UploadedFile = ({ files, onDeleteFile, isLoading, loadError }: UploadedFileProps) => {
  const getFileIcon = (fileType: string) => {
    switch (fileType) {
      case 'pdf':
        return <PDFIcon className="h-[18px] w-[17.44px]" />
      case 'txt':
          ...
          >
            <div className="flex h-full w-full items-center justify-between px-4">
              ...
              <button
                ...
                className="shrink-0 p-1 text-[#EF4444] transition-colors hover:text-red-600"
              >
