...
export const UserTable: React.FC<UserTableProps> = ({ users, loading, onEdit, onLock, onUnlock }) => {
        ...
        {users.map((user: User) => (
          <tr key={user.id} className="border-b border-slate-100 text-sm text-slate-700 last:border-b-0">
            ...
            <td className="px-6 py-4 whitespace-nowrap">
              <UserActions user={user} onEdit={onEdit} onLock={onLock} onUnlock={onUnlock} variant="desktop" />
            </td>
          </tr>
        ))}
      </>
