...
if (process.env.CONDA_PREFIX) {
  ...
  if (fs.existsSync(condaRuff) && fs.existsSync(condaBlack)) {
    ...
    console.warn(
      "⚠ Conda environment detected but ruff/black not found. Falling back to venv.",
    );
    condaRuff = null;
    condaBlack = null;
  }
...
const ruffPath =
  condaRuff || path.join(venvBinDir, isWindows ? "ruff.exe" : "ruff");
const blackPath =
  condaBlack || path.join(venvBinDir, isWindows ? "black.exe" : "black");
...
if (!condaRuff && !fs.existsSync(venvBinDir)) {
  ...
  console.error(
    '   Please run: cd apps/api && python -m venv venv && pip install -e ".[dev]"',
  );
  process.exit(1);
}
